    ...
    def _initialize_distr(
        ...
        if self.mixture_weights is None:
            self.distr = distributions.Independent(
                distributions.Normal(self.loc.squeeze(0), self.scale.squeeze(0), validate_args=False), 1, validate_args=False
            )
        else:
            modes = distributions.Independent(
                distributions.Normal(self.loc, self.scale, validate_args=False), 1, validate_args=False
            )
            mix = distributions.Categorical(self.mixture_weights, validate_args=False)
            self.distr = distributions.MixtureSameFamily(mix, modes, validate_args=False)
